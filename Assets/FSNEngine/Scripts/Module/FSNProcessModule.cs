...
/// </summary>
public interface IFSNProcessModule
{
	/// <summary>
	/// 해당 layer와 다음 명령어 조각을 사용해서 다음 layer 상태를 생성하여 리턴한다.
	/// 현재 Snapshot에 특정 명령어가 적용된 후의 Snapshot을 만들어내는 데 사용.
	/// </summary>
	FSNSnapshot.Layer GenerateNextLayerImage(FSNSnapshot.Layer curLayer, FSNSequence.Segment nextSeg);
}
	...
	where ObjT : FSNLayerObject<ElmT>
{
	...
	/// 현재 Snapshot에 특정 명령어가 적용된 후의 Snapshot을 만들어내는 데 사용.
	/// </summary>
	public FSNSnapshot.Layer GenerateNextLayerImage(FSNSnapshot.Layer curLayer, FSNSequence.Segment nextSeg)
	{
		return GenerateNextLayerImage(curLayer, nextSeg as SegT);
	}
}
